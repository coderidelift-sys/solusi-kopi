...
class TableDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addIndexColumn()
            ->addColumn('action', 'console.tables.action')
            ->editColumn('outlet_id', function (Table $table) {
                return $table->outlet->name ?? 'N/A';
            })
            ->editColumn('updated_at', function ($table) {
                return $table->updated_at->format('d F Y H:i');
            })
            ->editColumn('status', function ($table) {
                switch ($table->status) {
                    ...
                    case 'occupied':
                        return '<span class="badge bg-warning">Terisi</span>';
                }
    ...
    public function html(): HtmlBuilder
    {
        ...
        $buttons = [
            [
                'text' => '<i class="ri-add-line me-0 me-sm-1"></i><span class="d-none d-sm-inline-block">Tambah Meja</span>',
                'className' => 'add-new btn btn-primary mb-5 mb-md-0 me-3 waves-effect waves-light',
                'action' => 'function (e, dt, node, config) { window.location = "' . route('tables.create') . '"; }'
            ],
            [
                ...
                'action' => 'function (e, dt, node, config) { dt.ajax.reload(); $("#tables-table_filter input").val("").keyup(); }'
            ]
        ];
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('table_number')->title('Nomor Meja'),
